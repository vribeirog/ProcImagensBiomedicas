...
class silhouette():
    ...
    def _intra_cluster_distances_block_(subX):
        chunk_size = 10000
        num_points = subX.shape[0]
        total_distances = torch.zeros(num_points, device=torch.device("cuda"))
        
        for start_col in range(0, num_points, chunk_size):
            end_col = min(start_col + chunk_size, num_points)
            chunk = subX[start_col:end_col, :]
            distances = torch.cdist(subX, chunk)
            total_distances += distances.sum(axis=1)
        
        mean_distances = total_distances / (num_points - 1)
